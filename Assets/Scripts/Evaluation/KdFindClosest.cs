...
namespace Evaluation
{
    ...
    public class KdFindClosest : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject[] safepoints;
        ...
        [Header("Data")] private string RightControllerPos = @"c:\temp\RightContPos.txt";
    ...
    **/
        ...
        private void WithHead_threshold4()
        {
            ...
            foreach (var hand in Hands)
            {
                ...
                if ((Vector3.Distance(point.transform.position, hand.transform.position) < _threshold) &&
                    IsVisible(point.GetComponent<Renderer>()))
                {
                    //smallestf = dist;
                    nearestObj = point;
                }
        ...
        private SpawnedPoint Use_AnglesHandvel(List<SpawnedPoint> pt, Vector3 newpos, Vector3 raydirection,
            float lambda = Mathf.Infinity)
        {
        ...
        public Vector3 Testraycast()
        {
                ...
                Physics.DefaultRaycastLayers))
            {
                Debug.DrawRay(cam.transform.position,
                    cam.transform.TransformDirection(Vector3.forward) * hitInfo.distance, Color.green);
                return hitInfo.point;
            }
            else
            {
                Debug.DrawRay(cam.transform.position, cam.transform.TransformDirection(Vector3.forward) * 1000,
                    Color.red);
                return hitInfo.point;
