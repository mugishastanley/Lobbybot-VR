...
namespace Evaluation
{
    public class VelUDP2 : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject Workspaceplane;
        [SerializeField] private Transform Torso;
        [SerializeField] private int homecounter = 0;
        ...
        [SerializeField] private bool Sendhome;
    ...
    **/
        ...
        public float VelscalerTest(Vector3 point)
        {
            ...
            if (previous != point)
            {
                ...
                {
                    //we are inside the car
                    if ((previous.x > plane.x) ||
                        (Mathf.Abs(previous.x - plane.x) < tol)) //previous point outside or on plane
                    {
                        velfactor = 5f;
                        Debug.Log("previous was outside or on plane vel next is outside or on plane" +
                                  velfactor.ToString("F3") + "prev pos" + previous.x);
                    }
                    else
                    {
                        velfactor = 0.8f;
                        //    Debug.Log("else previous.x >= plane.x" + velfactor.ToString("F3"));
                        Debug.Log("previous was inside plane  next is on plane vel " + velfactor.ToString("F3") +
                                  "prev pos" + previous.x);
                    }
                }
                else
                {
                    ...
                    //Debug.Log("point.x < plane.x == plane.x " + plane.x.ToString("F3"));
                    //Debug.Log("point.x < plane.x == point.x " + point.x.ToString("F3"));
                    Debug.Log("previous inside plane next is also inside plane vel " + velfactor.ToString("F3") +
                              "prev pos" + previous.x);
        ...
        public float Velscaler3(Vector3 point, Transform torso) // with 3 worksapces
        {
            ...
            if (previous != point)
            {
                if (point.x <= plane.x)
                {
                    ...
                    if (((xl <= point.x) && (point.x <= xu)) &&
                        ((zl <= point.z) && (point.z <= zu))) //point inside WH space, hand approx as retangle.
                    {
                        velfactor = 0.1f;
                    }
                    else
